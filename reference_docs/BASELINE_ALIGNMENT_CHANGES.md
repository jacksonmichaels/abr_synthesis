...
## Follow-up (2026-07-28): the aligned protocol *collapsed* on imbalanced drugs — fixed

Running the aligned protocol end-to-end on MOXIFLOXACIN (388 R / 2480 S, 13.5% resistant)
made it **worse**, not better: CV AUC 0.59, TEST 0.71, **sens 0.00** (all-susceptible), and
the 60-epoch run early-stopped in ~1–9 epochs (31 s total). Diagnosis: the two aligned
choices interact badly on heavy imbalance —
- **output-bias `+log(n_S/n_R) ≈ +1.855`** starts the model already predicting the majority,
  so val-loss is *already* near its floor at epoch 0, and
- **`EarlyStopping(monitor='val_loss')`** then sees no loss improvement (the degenerate
  majority solution is a low-loss local optimum) and stops before the CNN learns the
  minority class.

A knob sweep on MOXI (`scripts/sweep_moxi.py`, 7 configs) isolated the fix — **the monitor
metric is the dominant lever, not patience**:

| config (monitor/patience/out_bias) | CV AUC | TEST | sens |
|---|---|---|---|
| **auc / 15 / None  ← new default** | **0.847** | **0.791** | 0.55 |
| auc / 15 / auto | 0.835 | 0.763 | 0.49 |
| auc / 5 / auto | 0.828 | 0.783 | 0.00 |
| loss / 999 / auto (full 60 ep) | 0.825 | 0.772 | 0.58 |
| loss / 5 / None | 0.655 | 0.731 | 0.00 |
| loss / 15 / auto | 0.606 | 0.713 | 0.00 |
| **loss / 5 / auto (shipped)** | 0.587 | 0.713 | 0.00 |

Production CLI re-check (`run_experiment.py`, the sbatch path): MOXI **CV 0.853, TEST 0.808,
sens 0.59** — gap to BIG-TB SD-CNN (0.886) down from −0.173 to **−0.078**.

**Changes made:**
- `train.py` `EarlyStopper` gained `mode` ('min' for loss, 'max' for AUC).
- `train_multimodal.py` `_train` gained `monitor` ('loss'|'auc', needs `y`) via new `_val_auc`;
  `run_modal_cv` gained `monitor`/`out_bias` params. **New defaults: `monitor='auc',
  patience=15, out_bias=None`** (was `'loss'`, 5, `'auto'`). `out_bias='auto'`/`monitor='loss'`
  still reproduce the strict baseline.
- `run_experiment.py` exposes `--monitor` / `--patience` / `--min-delta` / `--out-bias`
  (defaults match the new winner), so every sbatch job inherits the fix automatically.
- `test_baseline_alignment.py` #1/#9 now passes `out_bias='auto'` explicitly (the default is
  now None). Still 8/8.

This is a deliberate, evidence-backed deviation from the strict Keras baseline
(`monitor='val_loss'`), justified by the collapse it caused on the imbalanced drugs.
